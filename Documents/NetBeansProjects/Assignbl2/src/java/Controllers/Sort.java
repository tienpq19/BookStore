...
public class Sort extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        ArrayList<BookCate> bc = dao.getBookCate();
          
        if("new".equals(order)){
            ArrayList<Book> books=dao.getBooks();
        request.setAttribute("sort", "Newest");
        request.setAttribute("category", bc);
        request.setAttribute("books", books);
        request.getRequestDispatcher("View/category.jsp").forward(request, response);
        }else if("lth".equals(order)){
            ArrayList<Book> books=dao.getBooksbyPrice(order);
            request.setAttribute("sort", "Price:Low to High");
        request.setAttribute("category", bc);
        request.setAttribute("books", books);
        request.getRequestDispatcher("View/category.jsp").forward(request, response);
        }else{
             ArrayList<Book> books=dao.getBooksbyPrice(order);
            request.setAttribute("sort", "Price:High to Low");
        request.setAttribute("category", bc);
        request.setAttribute("books", books);
        request.getRequestDispatcher("View/category.jsp").forward(request, response);
        }
